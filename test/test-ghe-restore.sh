...
begin_test "ghe-restore reports an error when snapshot is incomplete"
(
  set -e
  touch "$GHE_DATA_DIR/1/incomplete"

  # output="$(ghe-restore -f localhost)" || false
  ghe-restore -f localhost -v > "$TRASHDIR/restore-out" 2>&1 || true
  # clean up the incomplete snapshot
  rm "$GHE_DATA_DIR/1/incomplete"

  grep -q "incomplete. Please restore from a complete snapshot." "$TRASHDIR/restore-out"
)
end_test

begin_test "ghe-restore to instance that is more than two versions ahead of snapshot"
(
  set -ex
  ...
  snap_ver=$(echo $GHE_TEST_REMOTE_VERSION | awk -F'.' '{gsub(/[a-zA-Z]/, "", $1); print $1"."$2 - 3}')

  # run ghe-restore and write output to file for asserting against
  SNAPSHOT_VERSION=$snap_ver ghe-restore -v -f > "$TRASHDIR/restore-out" 2>&1 || true

  # for debugging
  cat "$TRASHDIR/restore-out"

  grep -q "GitHub appliance being restored is more than two versions ahead of the backup snapshot" "$TRASHDIR/restore-out"
)
end_test
...
begin_test "ghe-restore uses ghe-license script from 3.13.0 onwards"
(
  set -e
  rm -rf "$GHE_REMOTE_ROOT_DIR"
  setup_remote_metadata

  output=$(GHE_REMOTE_VERSION=3.13.0 ghe-restore -v -f localhost)

  if ! echo "$output" | grep -q "License synchronized"; then
    echo "ghe-restore should use the new script ghe-license with import command." >&2
    exit 1
  fi
  exit 0
)
